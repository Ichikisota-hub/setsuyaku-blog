...
# ─── 信頼性・正確性ルール（全プロンプト共通） ──────────────
# 事実に基づいた正確で検証可能な情報のみを生成させ、推測や根拠の
# ない断定を避けるための共通指示。E-E-A-T（信頼性）を担保する。
RELIABILITY_RULES = """【信頼性・正確性ルール（厳守）】
- 事実に基づいた正確な情報のみを記載し、推測や憶測で内容を補完しない。
- 数値・金額・料金は、検証可能な一般的事実か、明確に「目安」「一例」と分かる形で示す。確実でない具体的数値を断定的に書かない。
- 公的機関（総務省・経済産業省・消費者庁・各事業者の公式情報など）のデータに基づく主張を優先し、可能な場合は出典（機関名・制度名）を本文中に明記する。
- 出典が一般論にとどまる場合は「一般的に」「事業者・契約により異なります」等で情報の限界を明示する。
- 料金・サービス内容・キャンペーンは変動するため、「最新の料金・条件は公式サイトで確認してください」と必ず促す。
- 十分な根拠がない事項は断定せず、確認を促す表現にとどめ、信頼できる情報がない場合は無理に記載しない。"""
...
- 1800〜2200字
- 具体的な数字・金額は検証可能な範囲で、不確実なものは「目安」と明示する
